...
**Goal:** Refactor the onboarding wizard from a centered card layout to a polished split-screen layout with Framer Motion animations.

**Architecture:** The wizard component (`onboarding-wizard.tsx`) is fully rewritten as a split-screen: fixed dark left panel (brand + step navigator with connector lines) and white right panel (form + nav buttons). All sub-components live in the same file. The layout.tsx is simplified (bare wrapper + success route exception). Framer Motion handles direction-aware step transitions (direction persisted via sessionStorage across page navigations), field stagger, and success confetti.

**Tech Stack:** Next.js App Router, React Hook Form + Zod, Framer Motion, canvas-confetti, TailwindCSS, shadcn/ui components (Input, Button, Textarea)
...
|---|---|---|
| `apps/web/src/components/onboarding/onboarding-wizard.tsx` | **Rewrite** | Full wizard: split-screen layout, sidebar with connector lines, 5 step forms, animations, confetti |
| `apps/web/src/app/onboarding/layout.tsx` | **Modify** | Simplify to bare wrapper + add success route exception to prevent redirect loop |
| `apps/web/package.json` | **Modify** | Add framer-motion, canvas-confetti, @types/canvas-confetti |

No backend changes. No new files. No test files (pure visual refactor — backend tests cover the API).
...
- [ ] **Step 1: Install framer-motion, canvas-confetti, and types**

```bash
cd apps/web && pnpm add framer-motion canvas-confetti && pnpm add -D @types/canvas-confetti
```

- [ ] **Step 2: Verify installation**

```bash
cd apps/web && node -e "require('framer-motion'); require('canvas-confetti'); console.log('OK')"
```
...
git commit -m "chore(web): add framer-motion + canvas-confetti for onboarding animations"
```

---

### Task 2: Update onboarding layout.tsx

**Files:**
- Modify: `apps/web/src/app/onboarding/layout.tsx`

Two changes: (1) simplify to bare wrapper (remove header + max-w-lg), (2) add success route exception — after `POST /onboarding/complete`, `isOnboarded` becomes true, so without this exception the layout redirects to `/dashboard` before the success page ever renders.
...
Changes:
- Removed `<header>` with PsyLib brand (moves into wizard sidebar)
- Removed `flex items-center justify-center` centering + `max-w-lg` constraint
- Added success route exception: skip `isOnboarded` redirect when URL ends with `/success`
- Used `headers()` to detect the current pathname (Next.js App Router provides this via `x-next-pathname` or we can parse the URL)

**Important:** If `x-next-pathname` is not available in your Next.js version, use an alternative approach — check the `[step]` page param instead. The simplest fallback is to pass a prop from the `[step]/page.tsx` to the layout, but since layouts don't receive params directly in App Router, the `headers()` approach is preferred. If it doesn't work, we'll fall back to checking `referer` or simply moving the redirect check into the `[step]/page.tsx` instead.
...
git commit -m "refactor(onboarding): simplify layout + add success route exception"
```

---

### Task 3: Rewrite onboarding-wizard.tsx (single atomic commit)

**Files:**
- Modify: `apps/web/src/components/onboarding/onboarding-wizard.tsx` — **full rewrite**

This is the main task. We replace the entire file in one go to avoid broken intermediate commits. The file is ~550 lines with all sub-components inlined.

**Key design decisions addressing review findings:**
- **Direction state:** Persisted via `sessionStorage` so it survives `router.push()` page navigation
- **Sidebar connector lines:** Vertical `motion.div` lines between step dots, animated with spring
- **Mobile safe area:** `pb-[max(12px,env(safe-area-inset-bottom))]` on sticky bottom nav
- **DurationChips:** Wired via React Hook Form `Controller` (not `register`)

- [ ] **Step 1: Replace the entire file**

Write the complete `onboarding-wizard.tsx`:
...
Expected: Build succeeds. If TypeScript errors appear (e.g. `loading` prop not on Button), check the existing Button component — the old code already uses `loading` so it should work.

- [ ] **Step 3: Commit**

```bash
git add apps/web/src/components/onboarding/onboarding-wizard.tsx
git commit -m "feat(onboarding): split-screen wizard with Framer Motion animations and confetti"
```
...
**Files:**
- Possibly tweak: `apps/web/src/components/onboarding/onboarding-wizard.tsx`

**Note:** Testing requires an authenticated session. Either log in first via `/login`, or temporarily bypass the auth check in `layout.tsx` during local development.

- [ ] **Step 1: Start dev server and walk through all 5 steps**

```bash
cd apps/web && pnpm dev
```

Navigate to `http://localhost:3000/onboarding/profile` and test each step:
1. Profile → fill name (min 2 chars), click Continue — verify slide animation
2. Practice → fill or skip, click Continue — verify tip box renders
3. Preferences → click a duration chip, adjust rate — verify chip selection highlights
4. First Patient → test both "Passer" and "Terminer 🎉"
5. Success → verify confetti fires, 3 quick action cards work, referral input works

- [ ] **Step 2: Test mobile responsive (375px width)**

Open browser DevTools, toggle to 375px. Verify:
- Top bar shows PsyLib + dots + step count (no sidebar)
- Form is full width with padding
- Sticky bottom nav visible with safe area padding
- No horizontal overflow

- [ ] **Step 3: Test back navigation direction**

On step 3, click "Retour" — verify content slides right (not left). The direction should reverse because `sessionStorage` stores `-1`.

- [ ] **Step 4: Test keyboard navigation**

Tab through all fields, press Enter to submit. Verify logical focus order.

- [ ] **Step 5: Fix any visual issues found and commit**

```bash
git add -u
git commit -m "fix(onboarding): visual polish after manual testing"
```
