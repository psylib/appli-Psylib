...
function ModuleItem({ module, index }: ModuleItemProps) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-border bg-white p-4 hover:border-primary/20 transition-colors">
      {/* Drag handle (visual only) */}
      <div
        className="flex-shrink-0 text-muted-foreground/40 cursor-grab active:cursor-grabbing"
        aria-hidden
        title="Réordonner (prochainement)"
      >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="bg-white rounded-xl border border-border p-4 flex flex-col gap-1">
                <div className="flex items-center gap-1.5 text-emerald-600 mb-1">
                  <Euro size={14} />
                  <span className="text-xs text-muted-foreground">Revenus générés</span>
                </div>
          ...
          <section aria-labelledby="modules-heading">
              ...
              {sortedModules.length === 0 && !showAddModule ? (
                <div className="rounded-xl border border-dashed border-border p-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    Aucun module pour l&apos;instant. Ajoutez du contenu à votre formation.
                  </p>
                  <button
                    onClick={() => setShowAddModule(true)}
            ...
            {sortedModules.length > 0 && (
              <p className="text-xs text-muted-foreground mt-3">
                La réorganisation par glisser-déposer sera disponible prochainement.
              </p>
            )}
          </section>
        </div>
