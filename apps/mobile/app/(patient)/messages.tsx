/**
 * Patient Messages — Chat with psychologist (reuses Socket.io)
 */
...
export default function PatientMessagesScreen() {
  ...
  const { data: messages } = useQuery({
    ...
    queryFn: () => apiClient.get<Message[]>('/patient-portal/messages', token ?? undefined),
    enabled: !!token,
  ...
  const sendMessage = useMutation({
    mutationFn: (content: string) =>
      apiClient.post<Message>('/patient-portal/messages', { content }, token ?? undefined),
    onSuccess: () => {
    ...
    <SafeAreaView style={styles.safeArea} edges={['bottom']}>
      ...
      >
        <FlatList
          ...
          inverted
          renderItem={({ item }) => {
            const isMine = item.senderId === patient?.id;
...
const styles = StyleSheet.create({
  ...
  bubbleTheirs: { backgroundColor: Colors.surfaceElevated, borderBottomLeftRadius: 4, borderWidth: 1, borderColor: Colors.border },
